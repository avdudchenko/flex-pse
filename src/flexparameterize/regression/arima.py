...
OLS / Levenberg-Marquardt backend that minimises the exact residual the
Pyomo surrogate implements.

**Restriction**: Only non-differenced AR/MA models are supported
(``d=0``, ``D=0``).  The Pyomo surrogate implements the mean ARIMA equation
in closed form, which requires no differencing.  Models with ``d>0`` or
``D>0`` will raise ``FlexConfigError``.
...
class ArimaRegressor:
    ...
    **Restriction**: Only non-differenced models are supported (``d=0``,
    ``D=0``).  The Pyomo surrogate implements the mean ARIMA equation in
    closed form, which cannot represent differencing.
    ...
    **AR persistence check**: After fitting, AR coefficients with absolute
    ...
    Raises:
        FlexConfigError: If ``d > 0`` or ``D > 0`` is requested, if
            ``include_drift=True`` is requested, or if any fitted AR
    ...
    def __init__(
        ...
        **auto_kwargs: object,
        ...
        if order is not None and order[1] != 0:
            raise FlexConfigError(
                f"ArimaRegressor only supports non-differenced models "
                f"(d=0). Got order={order} with d={order[1]}. "
                f"Use order=(p, 0, q) instead.",
                field="order",
                value=order,
            )
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> ArimaRegressor:
        ...
        **Restriction**: Only non-differenced models are fitted (``d=0``,
        ...
        if self._auto:
            ...
            min_rows = (self._order or (0, 0, 0))[2] + 1
        ...
        residual_ss = float(np.nansum((y_values - fitted_values) ** 2))
...
def _extract_order(fitted_model) -> tuple[int, int, int]:
    """Return ``(p, d, q)`` from a fitted direct or statsmodels result.
...
class _DirectModel:
    """Mimics the statsmodels SARIMAX model interface."""
...
class _DirectResults:
    """Mimics the statsmodels SARIMAXResults interface.
    ...
    def __init__(
        ...
        self._model = _DirectModel(param_names, k_ar, k_ma, k_diff, seasonal_order)
    ...
    def predict(
        ...
        Args:
            ...
            dynamic: If ``True`` (default), use recursive predictions where
                each predicted value feeds back as an AR lag.  If ``False``,
                use actual observed values as lags (one-step-ahead prediction,
                like ``fittedvalues``).
        ...
        has_const = True  # always present in our parameterisation
        ...
        if start is not None:
            ...
            y_hist = list(self._y[max(0, start - p) : start])
            eps_hist = list(self._resid[max(0, start - q) : start]) if q > 0 else []
            forecasts = []

            for step in range(steps):
                ar_part = sum(ar[j] * y_hist[-(j + 1)] for j in range(p))
                # For in-sample simulation, include MA terms from training residuals
                # at all steps (matching Pyomo surrogate behavior)
                ma_part = (
                ...
                y_hist.append(y_hat)
                # For in-sample simulation, append the training residual
                # (matching Pyomo surrogate's stored residuals)
        ...
        y_hist = list(self._y[-p:] if p > 0 else [])
        ...
        for step in range(steps):
            ar_part = sum(ar[j] * y_hist[-(j + 1)] for j in range(p))
            ma_part = (
                sum(ma[j] * eps_hist[-(j + 1)] for j in range(q)) if step == 0 else 0.0
            )
            exog_part = (
                sum(beta[k] * exog[step, k] for k in range(n_exog))
                if exog is not None
    ...
    *,
    ...
    if P > 0 or D > 0 or Q > 0:
        raise NotImplementedError(
            f"Seasonal ARIMA terms are not yet supported in the direct "
            f"fit backend. Got seasonal_order={seasonal_order}."
        )
